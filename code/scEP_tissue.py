...
class scEPdata(Dataset):
    def __init__(self, general_fn, exp_fn, col, idx, idx_train, upstream=7000, downstream=3500):
        ...
        train_labels = all_labels.loc[gn_train].values[:,col]
        labels = all_labels.loc[gn].values[:,col]
        scaler = StandardScaler()
        scaler.fit(train_labels.reshape((-1,1)))
        
        self.labels = np.squeeze(scaler.transform(labels.reshape((-1,1))))
...
class CNN1D(nn.Module):
    def __init__(self, upstream=7000, downstream=3500):
        ...
        fc_layers.append(nn.Linear(64, 2))
        fc_layers.append(nn.ReLU())
        fc_layers.append(nn.Dropout(p=0.01546))
        fc_layers.append(nn.Linear(2,1))
