...
from scEP_tissue import scEPdata, CNN1D, train, test
...
parser.add_argument('--col',            dest='col',             type=int,       default=0,
                    help='Index of the column you want to train the model for')
...
for train_val_idx, idx_test in kf.split(idx_all):
    if counter == startfold:
        ...
        # Create dataloaders for training, validation, test set
        input_data = general_dir + train_file
        train_set = scEPdata(input_data, label_fn, col, idx_train, idx_train,
                             upstream=upstream, downstream=downstream)
        train_loader = DataLoader(train_set, batch_size=128, shuffle=True)
        train_loader_eval = DataLoader(train_set, batch_size=1, shuffle=False)
        
        val_set = scEPdata(input_data, label_fn, col, idx_val, idx_train,
                           upstream=upstream, downstream=downstream)
        val_loader = DataLoader(val_set, batch_size=1, shuffle=False)
        
        test_set = scEPdata(input_data, label_fn, col, idx_test, idx_train,
                            upstream=upstream, downstream=downstream)
        ...
        while i < numruns:
            ...
            net = CNN1D(upstream=upstream, downstream=downstream).to(device)
